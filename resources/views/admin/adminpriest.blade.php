...
<div class="container-fluid">
    ...
    <div class="btn-group">
        <button type="button" id="addbutton" class="btn btn-dark shadow-sm btn-sm" data-mdb-toggle="modal"
            data-mdb-target="#addeditmodal"><i class="fa-solid fa-circle-plus"></i> Add A New Priest</button>
    </div>
        ...
        <div class="col">
            <form method="get">
                ...
                <div class="input-group mb-3">
                    ...
                    @if(!empty($keyword))
                    <button class="btn btn-dark" type="button" onclick="location.href='./adminpriest'"><i
                ...
                <div class="input-group mb-3">
                    ...
                    <button onclick="location.href='./adminpriest'" type="button" class="btn btn-dark"><i
                            class="fas fa-search fa-rotate fa-sm"></i></button>
                    @endif
            ...
            <table class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <th scope="col">
                            <span
                                class="{{ $orderbylist[$sort]['display'] == 'Position' ? 'text-primary' : '' }}"><strong>Position</strong></span>
                        </th>
                        <th scope="col">
                            <span
                                class="{{ $orderbylist[$sort]['display'] == 'Conventual' ? 'text-primary' : '' }}"><strong>Conventual</strong></span>
                        </th>
                        <th scope="col">
                            <span
                        ...
                        <td>
                            <div class="btn-group" role="group">
                                ...
                                <a href="#" class="btn btn-dark btn-sm azu-lock" data-id="{{$dbr->id}}"
                                    data-email="{{$dbr->email}}" data-mdb-toggle="modal" data-mdb-target="#lockmodal">
...
<script type="text/javascript">
    $(document).ready(function(){
        $('#addbutton').on('click', function(){
            $('#addeditmodalLabel').html('New Priest Profile');
            $('#addeditframe').attr('src', '/adminpriest_add');
        })
        $('.azu-edit').on('click', function(){
            var sid = $(this).data("id");
            $('#addeditmodalLabel').html('Edit Priest Profile');
            $('#addeditframe').attr('src', '/adminpriest_edit?id='+sid);
        })
        $('.azu-lock').on('click', function(){
            var email = $(this).data("email");
            var sid = $(this).data("id");
            $('#lockmodallabel').html('Disable Priest');
            $('#modalmethod').html('inactive');
            $('#modalemail').html(email);
            $('#DeleteButton').prop('href', '/adminpriest_lock_process?id='+sid);
        })
        $('.azu-unlock').on('click', function(){
            var email = $(this).data("email");
            var sid = $(this).data("id");
            $('#lockmodallabel').html('Activate Priest');
            $('#modalmethod').html('active');
            $('#modalemail').html(email);
            $('#DeleteButton').prop('href', '/adminpriest_unlock_process?id='+sid);
        })
    })
</script>
@endpush