...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col">
            <h1>Parish Articles</h1>
        </div>
    </div>

    <div class="btn-group">
        <button type="button" id="addbutton" class="btn btn-dark shadow-sm btn-sm" data-mdb-toggle="modal"
            data-mdb-target="#addeditmodal"><i class="fa-solid fa-circle-plus"></i> Add A New Article</button>
    </div>
            ...
            <table class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <th scope="col">
                            <span
                                class="{{ $orderbylist[$sort]['display'] == 'Title' ? 'text-primary' : '' }}"><strong>Title</strong></span>
                        </th>
                        <th scope="col">
                            <span
                                class="{{ $orderbylist[$sort]['display'] == 'Content' ? 'text-primary' : '' }}"><strong>Content</strong></span>
                        </th>
                        <th scope="col">
                ...
                <tbody>
                    ...
                    <tr class="{{$dbr->status == 'inactive' ? 'table-danger' : ''}}">
                        ...
                        <td>{{$dbr->title}}</td>
                        <td>{{ \Illuminate\Support\Str::limit($dbr->content, 50) }}</td>
...
<div class="modal fade" id="lockmodal" data-mdb-backdrop="static" data-mdb-keyboard="false" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                Are you sure you want to make this article <span id="modalmethod">inactive</span>?<br>
...
<script type="text/javascript">
    $(document).ready(function(){
        $('#addbutton').on('click', function(){
            $('#addeditmodalLabel').html('New Parish Article');
            $('#addeditframe').attr('src', '/adminarticle_add');
        })
        ...
        $('.azu-lock').on('click', function(){
            ...
            var sid = $(this).data("id");
            $('#lockmodallabel').html('Disable Article');
            $('#modalmethod').html('inactive');
            $('#modalemail').html(email);
            $('#DeleteButton').prop('href', '/adminarticle_lock_process?id='+sid);
        })
        $('.azu-unlock').on('click', function(){
            var email = $(this).data("email");
            var sid = $(this).data("id");
            $('#lockmodallabel').html('Activate Article');
            $('#modalmethod').html('active');
            $('#modalemail').html(email);
            $('#DeleteButton').prop('href', '/adminarticle_unlock_process?id='+sid);
        })
    })
</script>
@endpush